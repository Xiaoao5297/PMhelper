	...
	public function jishi(){
		if($this->gamestart){
			...
			switch($this->shijian2){
				...
				$this->gamestart = false;
				$this->cx = 4;
				$p11->sendTip(CL::GREEN."§e▎[§6！§e]§6➢§f[极速点击Ⅱ] 游戏结束!");
				$p11->sendMessage(CL::GREEN."§e▎[§6！§e]§6➢§f[极速点击Ⅱ] 你一共点击了".($this->css)."次.");
				$this->players = array();
				$this->css = 0;
				$this->csign();
				break;
		...
		if($c->getName() == "scg"){
			if(isset($args[0])){
				if($s->isOp()){
					...
					$s->sendMessage(CL::GREEN."§4▎[§c✘§4]§c➢§4[极速点击Ⅱ] 你没有权限执行此操作!");
				}
			}
		}
	}
			...
			if(!$this->gamestart){
				if($this->cfg->exists("sign")){
					...
					if($xx == $x1 and $yy = $y1 and $zz == $z1 and $level->getFolderName() == $leveln){
						$p->sendMessage(CL::GREEN."§2▎[§a✔§2]➢§2[极速点击Ⅱ] 你已加入游戏!");
					    $this->gamestart = true;
			...
			if($this->gamestart){
				...
				if($xx == $x1 and $yy = $y1 and $zz == $z1 and $level->getFolderName() == $leveln){
					if($pn == $this->players['id']){
						$this->css++;
						$p->sendMessage(CL::GREEN."§e▎[§6！§e]§6➢§e[极速点击Ⅱ] 你一共点了".($this->css)."次.");
					}
				}
			}
			break;
		...
		if($this->cfg->exists("sign1")){
			...
			if($sign instanceof Sign){
				if($this->css > $signtext[3]){
					...
					Server::getInstance()->broadcastMessage(CL::YELLOW."§e▎[§6！§e]§6➢§e[极速点击Ⅱ] ".($this->players['id'])."获得了极速点击Ⅱ第一名!");
				}
			}
