...
class YamlDatabase implements Database{
	...
	public function canPutWater($x,$z, $level,$name){
				foreach($this->land as $land){
					$bool=
					($level === $land["level"] and $land["startX"] < ($x+10) and $land["endX"] > ($x+10) and $land["startZ"] < $z and $land["endZ"] > $z)
				||  ($level === $land["level"] and $land["startX"] < ($x-10) and $land["endX"] > ($x-10) and $land["startZ"] < $z and $land["endZ"] > $z)
				||  ($level === $land["level"] and $land["startX"] < $x and $land["endX"] > $x and $land["startZ"] < ($z-10) and $land["endZ"] > ($z-10))	
				||  ($level === $land["level"] and $land["startX"] < $x and $land["endX"] > $x and $land["startZ"] < ($z+10) and $land["endZ"] > ($z+10))
				||  ($level === $land["level"] and $land["startX"] < ($x-10)and $land["endX"] > ($x-10) and $land["startZ"] < ($z-10) and $land["endZ"] > ($z-10))	
				||  ($level === $land["level"] and $land["startX"] < ($x+10) and $land["endX"] > ($x+10) and $land["startZ"] < ($z+10) and $land["endZ"] > ($z+10))
				||  ($level === $land["level"] and $land["startX"] < ($x-10)and $land["endX"] > ($x-10) and $land["startZ"] < ($z+10) and $land["endZ"] > ($z+10))	
				||  ($level === $land["level"] and $land["startX"] < ($x+10) and $land["endX"] > ($x+10) and $land["startZ"] < ($z-10) and $land["endZ"] > ($z-10));
	...
	if($bool)
				...
				if($name === $land["owner"] or isset($land["invitee"][$name])){ // If owner is correct
