...
Server::getInstance()->broadcastMessage("§7 > §f已清理生物和掉落物！");
      unset($i,$entity);
    }
...
class cleaner extends PluginTask
{
    ...
    public function onRun($currentTick)
    {
    	...
    	if($this->tmp==100)
    	{
    		Server::getInstance()->broadcastMessage("§7 > §f将在§4❺秒§f后进行清理");
    	}
    	if($this->tmp==200)
    	{
    		Server::getInstance()->broadcastMessage("§7 > §f将在§6❿秒§f后进行清理");
    	}
    	if($this->tmp==400)
    	{
    		Server::getInstance()->broadcastMessage("§7 > §f将在§2⓴秒§f后进行清理");
    	}
    }
