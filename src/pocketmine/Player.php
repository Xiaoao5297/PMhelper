	...
	public function handleDataPacket(DataPacket $packet){
		...
		switch($packet::NETWORK_ID){
			...
			case ProtocolInfo::CRAFTING_EVENT_PACKET:
				...
				$canCraft = true;//0.13.1大量物品本地配方出现问题,无法解决,使用极端(唯一)方法修复.←疑似前人的足迹
				$ingredients = $packet->input;
				$result = $packet->output[0];

				if(!$canCraft or !$recipe->getResult()->deepEquals($result)){
					$this->server->getLogger()->debug("未匹配的配方 " . $recipe->getId() . " 来自玩家 " . $this->getName() . ": 预期 " . $recipe->getResult() . ", 实际得到 " . $result . ", 使用材料: " . implode(", ", $ingredients));
					$this->inventory->sendContents($this);
					break;
				}
				...
				if(!$canCraft){
					$this->server->getLogger()->debug("配方不匹配 " . $recipe->getId() . " 来自玩家 " . $this->getName() . ": 客户端物品不足，使用材料: " . implode(", ", $ingredients));
